...
### Slide 7: The Lie Detector / How it Works

**File:** `figures/lie_detector_chart.png` (generated by `scripts/lie_detector_chart.py`)

**Chart:** Real trade data — raw ticks (dots), Kalman-smoothed baseline (teal line), volume bars. Green band = "Toxic Fill Detected" (price jump + volume spike). Regenerate with `--market mlb` to get an MLB example for the line: *"Here is an example from a flagged move during an MLB game."*

**Suggested script (referencing the plot):** "The detector constantly monitors the trade stream in real time. If order volume or price spikes relative to a recent rolling baseline, we pull quotes. The price change is more important than the order volume. *[Turn to chart]* Here is an example from a flagged move during an MLB game—the dots are raw trade prices, the teal line is our Kalman-smoothed baseline. When price jumps and volume spikes together, we flag it—that green band—and pull quotes. Notice the second volume spike didn't trigger a flag because there was no corresponding jump in price."

**What it shows (pipeline):**
